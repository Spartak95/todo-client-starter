...
public class ToDoClient {
    ...
    private final ToDoClientProperties props;

    public ToDo add(ToDo toDo) {
        UriComponents uriComponents = UriComponentsBuilder.newInstance()
            .uri(URI.create(this.props.getHost())).path(this.props.
                                                            getPath()).build();
        ResponseEntity<ToDo> response = this.restTemplate
            .exchange(RequestEntity.post(uriComponents.toUri()).body(toDo),
    ...
    public Collection<ToDo> findAll() {
        ...
        Traverson traverson = new Traverson(uriComponents.toUri(),
                                            MediaTypes.HAL_JSON, MediaType.APPLICATION_JSON_UTF8);
